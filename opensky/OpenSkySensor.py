...
class OpenSkySensor(SensorX):
    ...
    def get_trackdir(f_track):
        ...
        try:
            ...
            for degree in OpenSkySensor.TRACK_LIST:
                if abs(int(degree) - f_track) <= 1:
                    true_track = OpenSkySensor.TRACK_LIST[degree]
                elif (int(degree) > f_track) and (int(degree) - f_track) < 45:
                    true_track = OpenSkySensor.TRACK_LIST[degree]
                elif f_track > 315:
                    true_tract = OpenSkySensor.TRACK_LIST['315']
    ...
    def get_typeofaircraft(ft_dict):
        ...
        f_type = {}
        with open(os.path.join(os.path.dirname(__file__),'Aircrafts.json')) as code_list:
    ...
    def _create_content(self,fs_json):
        ...
        try:
            ...
            for v in fs_json['states']:
                ...
                f1= f_aircraft['Vectors']['GeoAlt']
                f2 = f_aircraft['Vectors']['Speed']
                ...
                aircraft_sum = 'It\'s cruising towards {} degrees, ({}) at {:.2f} ft. altitude with speed of {:.2f} '\
                               'mi/hr'.format(
                                ...
                                (f1 * OpenSkySensor.CONV_ALT) if isinstance(f1, float) else 0,
                                (f2 * OpenSkySensor.CONV_SPD) if isinstance(f2, float) else 0,
                )

                aircraft_vertPos = '. It\'s currently ' + vert_Pos + ' at the rate of {:0.2f} ft/s'.format(
                                                     f_aircraft['Vectors']['VertRate'] * OpenSkySensor.CONV_ALT)
                ...
                f_out = {'k': f_aircraft['icao24'],
                         ...
                         'img': os.path.join(os.path.dirname(__file__), 'images', self.props['background_img'])
                         }
